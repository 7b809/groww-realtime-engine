...
async def get_live_signal(
    ...
    if not latest:
        return {"message": "No latest candle found"}
    ...
    if symbol in last_processed_candle:
        if last_processed_candle[symbol] == latest_timestamp:
            return {
                "symbol": symbol,
    ...
    # Store latest
    last_processed_candle[symbol] = latest_timestamp

    # ==========================
    # FETCH LAST 24 HRS FOR CONTEXT
    # ==========================
    candles, _ = await fetch_last_24hrs(symbol, exchange)
